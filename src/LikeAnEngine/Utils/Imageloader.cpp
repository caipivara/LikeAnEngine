...
GLuint* getGLTextureEnvMode(int size){
    int *arrayMode = new int [size];
    for(int i = 0; i < size; i++){
        arrayMode[i] = GL_MODULATE;
    }

    return (GLuint*)arrayMode;
}
...
void loadTextures(Image *images, GLuint *texturesId, GLuint * glTextureEnvMode, int numText) {
  ...
  for(int i = 0; i < numText; i++){
    ...
    glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, glTextureEnvMode[i] );//MODULATE, DECAL, BLEND, REPLACE
